...
Raise the engineering baseline for the legacy Traveller Android app by making dependency resolution deterministic, removing an insecure Gradle wrapper URL, documenting the missing local configuration required by the Parse-backed app, and adding a lightweight source check that can run without an Android SDK.

---

## Problem Frame

The repository is a 2014-era Android project with Gradle 1.10, Android Gradle Plugin 0.8.x, Parse 1.5.0, dynamic dependency versions, and almost no setup documentation. A broad Android migration is high risk without a reproducible starting point, especially because this environment does not provide an Android SDK and the app depends on an ignored `Constants.java` file for Parse credentials.
...
- R2. The Gradle wrapper distribution URL must use HTTPS instead of HTTP.
- R3. The README must document the nested project path, legacy SDK/build-tools expectations, Parse credential file requirement, and local verification commands.
- R4. The repository must include a safe Parse credential template without committing real credentials.
- R5. A local source check must run without Android SDK installation and verify the reproducibility baseline.
- R6. Local verification results must distinguish source-check success from Android SDK/toolchain prerequisites.
- R7. Larger migrations to modern Android Gradle Plugin, AndroidX, maintained Parse alternatives, and real Android tests must remain explicit follow-up work.
...
- **Use HTTPS for wrapper download without changing Gradle:** Keeping Gradle 1.10 avoids widening the change while removing an avoidable transport risk.
- **Keep Parse secrets out of git:** Provide `Constants.java.example` and keep real `Constants.java` ignored.
- **Add SDK-free checks:** A shell script can validate pinned dependency declarations and required template/docs even when `./gradlew` cannot configure without an Android SDK.
...
- **Files:** `traveller-android-app/build.gradle`, `traveller-android-app/traveller/build.gradle`, `traveller-android-app/gradle/wrapper/gradle-wrapper.properties`
- **Patterns:** Keep the existing Gradle 1.10 / Android Gradle Plugin 0.8 project shape; replace only dynamic versions and the wrapper transport URL.
- **Test Scenarios:**
...
- **Test Scenarios:**
  ...
  - The script fails if the Parse credential template is missing.
  - The script succeeds in this environment without Android SDK configuration.
...
- `traveller-android-app/build.gradle` contains the dynamic Android Gradle Plugin version.
